...
AudioEngine::AudioEngine(QObject* parent) : QObject(parent)
{
    ...
    QObject::connect(m_audioControls, SIGNAL(musicSliderMoved(int, int, int)), m_musicPlayer, SLOT(onTrackSliderMoved(int, int, int)));
    ...
    QObject::connect(m_musicPlayer, SIGNAL(playlistEnded()), m_audioControls, SLOT(onPlaylistEnded()));
    QObject::connect(m_musicPlayer, SIGNAL(positionChanged(qint64, qint64)), m_audioControls, SLOT(onPositionChanged(qint64, qint64)));
}
